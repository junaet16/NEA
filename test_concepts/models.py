...
class StationModel(Base):
    ...
    start_routes = relationship("StandardRouteModel", foreign_keys="StandardRouteModel.StartStationID", back_populates="start_station")
    end_routes = relationship("StandardRouteModel", foreign_keys="StandardRouteModel.EndStationID", back_populates="end_station")
...
class UserModel(Base):
    ...
    routes = relationship("StandardRouteModel", back_populates="user")


class StandardRouteModel(Base):
    __tablename__ = "StandardRoutes"

    RouteID = Column(Integer, primary_key=True, autoincrement=True)
    UserID = Column(Integer, ForeignKey("Users.UserID"))
    StartStationID = Column(Text, ForeignKey("Stations.NaPTAN"))
    EndStationID = Column(Text, ForeignKey("Stations.NaPTAN"))

    user = relationship("UserModel", back_populates="routes")
    start_station = relationship("StationModel", foreign_keys=[StartStationID], back_populates="start_routes")
    end_station = relationship("StationModel", foreign_keys=[EndStationID], back_populates="end_routes")
...
class TeamModel(Base):
    ...
    VenueName = Column(Text, ForeignKey("Venues.VenueName"))